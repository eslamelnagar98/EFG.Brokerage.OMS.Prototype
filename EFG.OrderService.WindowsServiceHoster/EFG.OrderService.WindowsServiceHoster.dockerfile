...
WORKDIR /app
COPY . .
RUN msbuild EFG.OrderService.WindowsServiceHoster.csproj -t:restore -p:Configuration=Release
...
#r.microsoft.com/dotnet/framework/runtime:4.8
...
RUN "C:/Windows/Microsoft.NET/Framework64/v4.0.30319/installutil.exe" \
 ...
 C:/app/EFG.OrderService.WindowsServiceHoster.exe ;
